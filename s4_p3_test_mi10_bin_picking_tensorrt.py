...
def build_comparison(results_pytorch, results_tensorrt, obj_id):
    summary = {
        ...
        'tensorrt_time_s': float(results_tensorrt.get('time', 0.0)),
    }
    pred_pytorch = results_pytorch.get(obj_id, {})
    pred_tensorrt = results_tensorrt.get(obj_id, {})
    if not isinstance(pred_pytorch, dict) or not isinstance(pred_tensorrt, dict):
        summary['status'] = 'missing_obj_result'
        return summary

    if 'pred_mask_logits' in pred_pytorch and 'pred_mask_logits' in pred_tensorrt:
        mask_logits_diff = torch.abs(pred_pytorch['pred_mask_logits'] - pred_tensorrt['pred_mask_logits'])
        summary['mask_logits_mae'] = float(mask_logits_diff.mean().item())
        summary['mask_logits_max'] = float(mask_logits_diff.max().item())
    if 'pred_front_back_code_logits' in pred_pytorch and 'pred_front_back_code_logits' in pred_tensorrt:
        code_logits_diff = torch.abs(pred_pytorch['pred_front_back_code_logits'] - pred_tensorrt['pred_front_back_code_logits'])
        summary['code_logits_mae'] = float(code_logits_diff.mean().item())
        summary['code_logits_max'] = float(code_logits_diff.max().item())

    if 'pred_mask' in pred_pytorch and 'pred_mask' in pred_tensorrt:
        mask_binary_match = (pred_pytorch['pred_mask'] == pred_tensorrt['pred_mask']).to(torch.float32)
    ...
    Rts_tensorrt = np.asarray(pred_tensorrt.get('Rts', []), dtype=np.float32)
    pose_count = int(min(len(Rts_pytorch), len(Rts_tensorrt)))
    summary['pose_count_compared'] = pose_count
    pose_diffs = []
    for pose_id in range(pose_count):
        Rt_pt = Rts_pytorch[pose_id]
        Rt_trt = Rts_tensorrt[pose_id]
        trans_mm = float(np.linalg.norm(Rt_pt[:3, 3] - Rt_trt[:3, 3]))
        rot_deg = rotation_error_deg(Rt_pt[:3, :3], Rt_trt[:3, :3])
        pose_diffs.append({
            'pose_id': pose_id,
...
if __name__ == '__main__':
    ...
    hccepose_acceleration_tensorrt = 'tensorrt'
    ...
    tensorrt_tester = Tester(bop_dataset_item, hccepose_vis=hccepose_vis, CUDA_DEVICE=CUDA_DEVICE, hccepose_acceleration=hccepose_acceleration_tensorrt)
    ...
    for name in ['IMG_20251007_165718',
        ...
        results_tensorrt = tensorrt_tester.predict(cam_K, image, [obj_id], conf=0.85, confidence_threshold=0.85)
        print_stage_time_breakdown(results_pytorch, enabled=print_stage_timing, prefix='%s | PyTorch' % name)
        print_stage_time_breakdown(results_tensorrt, enabled=print_stage_timing, prefix='%s | TensorRT' % name)
        ...
        save_visual_artifacts([
            ...
            (file_name.replace('.jpg', '_tensorrt_show_2d.jpg'), results_tensorrt.get('show_2D_results')),
            (file_name.replace('.jpg', '_tensorrt_show_6d_vis0.jpg'), results_tensorrt.get('show_6D_vis0')),
            (file_name.replace('.jpg', '_tensorrt_show_6d_vis1.jpg'), results_tensorrt.get('show_6D_vis1')),
            (file_name.replace('.jpg', '_tensorrt_show_6d_vis2.jpg'), results_tensorrt.get('show_6D_vis2')),
        ], enabled=save_visualizations)

        if save_visualizations:
            compare_vis = hstack_images([
                add_title(results_pytorch.get('show_6D_vis1'), 'PyTorch'),
                add_title(results_tensorrt.get('show_6D_vis1'), 'TensorRT'),
            ])
            save_visual_artifacts([
                (file_name.replace('.jpg', '_compare_pytorch_tensorrt_6d.jpg'), compare_vis),
            ], enabled=True)

        comparison = build_comparison(results_pytorch, results_tensorrt, obj_id)
        with open(file_name.replace('.jpg', '_compare_pytorch_tensorrt.json'), 'w', encoding='utf-8') as f:
